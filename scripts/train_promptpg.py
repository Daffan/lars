...
from src.PromptPG.prompt_pg.base_prompt import *
from src.PromptPG.prompt_pg.model import *
from src.PromptPG.prompt_pg.utilities import extract_prediction, normalize_answer

from src.PromptPG.tools import utils
...
def get_gpt3_output(llm, prompt, args):
    return call_gpt3(llm, prompt, args.temperature, args.max_tokens, args.top_p, args.frequency_penalty,
                     args.presence_penalty)


# @lru_cache(maxsize=10000)
def call_gpt3(llm, prompt, temperature, max_tokens, top_p, frequency_penalty, presence_penalty):
    parameters = dict(
...
def get_batch_reward_loss(llm, scores, cand_pids, pid_batch, option_batch, unit_batch, label_batch, args):
    ...
    for i in range(len(scores)):
        ...
        output = llm.predict(prompt)  # get_gpt3_output(llm, prompt, args)
...
def policy_gradient_train(llm, policy_model, problems, train_pids, cand_pids, cand_examples, args):
    # REINFORCE
    ...
    for epoch in range(args.epochs):
        ...
        for batch_i in range(num_batch):
            ...
            # temperature = 10.0
            # scores = F.softmax(scores / temperature, dim=1)  # len(train_batch) x len(cand_examples)
            scores = torch.exp(F.log_softmax(scores, dim=1))  # len(train_batch) x len(cand_examples)

            cids, reward, loss = get_batch_reward_loss(llm, scores, cand_pids, pid_batch, option_batch, unit_batch,
                                                       label_batch, args)
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default='assets/dataset/tablemwp')
    ...
    parser.add_argument('--ckpt_root', type=str, default='results/prompt_pg/')
...
if __name__ == '__main__':
    ...
    llm_class = endpoint_to_class[args.engine]
    stop = ["\n"]  # TODO: check this
    llm = llm_class(
        args.engine,
        temperature=args.temperature,
    ...
    policy_gradient_train(llm, policy_model, problems, train_pids, cand_pids, cand_examples, args)
